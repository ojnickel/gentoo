...
complete -c flac -l 'analyze\\f' -d 'Analyze a FLAC encoded file (same as -d except an analysis file is written)'
...
complete -c flac -l 'warnings-as-errors\\f' -d 'Treat all warnings as errors (which cause flac to terminate with a non-zero …'
...
complete -c flac -l 'decode-through-errors\\f' -d 'By default flac stops decoding with an error and removes the partially decod…'
complete -c flac -l 'verify\\f' -d 'Verify a correct encoding by decoding the output in parallel and comparing t…'
complete -c flac -l seekpoint -d 'Include a point or points in a SEEKTABLE'
complete -c flac -l padding -d 'Tell the encoder to write a PADDING metadata block of the given length (in b…'
complete -c flac -l tag -d 'Add a FLAC tag.  The comment must adhere to the Vorbis comment spec; i. e'
...
complete -c flac -l max-lpc-order -d 'Specifies the maximum LPC order.  This number must be <= 32'
complete -c flac -l 'qlp-coeff-precision-search\\f' -d 'Do exhaustive search of LP coefficient quantization (expensive!)'
complete -c flac -l qlp-coeff-precision -d 'Precision of the quantized linear-predictor coefficients, 0 => let encoder d…'
complete -c flac -l rice-partition-order -d 'Set the [min,]max residual partition order (0. 15)'
complete -c flac -s 8 -d 'actually are just synonyms for different groups of specific encoding options…'
complete -c flac -s d -s a -s t -d 'Depending on which way is chosen, encoding, decoding, analysis or testing op…'
...
complete -c flac -l analyze -d 'Analyze a FLAC encoded file (same as -d except an analysis file is written)'
...
complete -c flac -s w -l warnings-as-errors -d 'Treat all warnings as errors (which cause flac to terminate with a non-zero …'
...
complete -c flac -l preserve-modtime -d 'Output files have their timestamps/permissions set to match those of their i…'
complete -c flac -l keep-foreign-metadata -d 'If encoding, save WAVE, RF64, or AIFF non-audio chunks in FLAC metadata'
complete -c flac -l keep-foreign-metadata-if-present -d 'Like --keep-foreign-metadata, but without throwing an error if foreign metad…'
...
complete -c flac -l serial-number -d 'When used with --ogg, specifies the serial number to use for the first Ogg F…'
complete -c flac -l residual-text -d 'Includes the residual signal in the analysis file'
complete -c flac -l residual-gnuplot -d 'Generates a gnuplot file for every subframe; each file will contain the resi…'
complete -c flac -l cue -d 'Set the beginning and ending cuepoints to decode.  The optional first #'
complete -c flac -s F -l decode-through-errors -d 'By default flac stops decoding with an error and removes the partially decod…'
...
complete -c flac -s V -l verify -d 'Verify a correct encoding by decoding the output in parallel and comparing t…'
complete -c flac -l lax -d 'Allow encoder to generate non-Subset files'
complete -c flac -l replay-gain -d 'Calculate ReplayGain values and store them as FLAC tags, similar to vorbisga…'
complete -c flac -l cuesheet -d 'Import the given cuesheet file and store it in a CUESHEET metadata block'
...
complete -c flac -l ignore-chunk-sizes -d 'When encoding to flac, ignore the file size headers in WAV and AIFF files to…'
complete -c flac -s S -d 'Include a point or points in a SEEKTABLE'
complete -c flac -s P -d 'Tell the encoder to write a PADDING metadata block of the given length (in b…'
complete -c flac -s T -d 'Add a FLAC tag.  The comment must adhere to the Vorbis comment spec; i. e'
complete -c flac -l tag-from-file -d 'Like --tag, except FILENAME is a file whose contents will be read verbatim t…'
complete -c flac -s b -d 'Specify the blocksize in samples.  The default is 1152 for -l 0, else 4096'
...
complete -c flac -l fast -d 'Fastest compression.  Currently synonymous with -0'
complete -c flac -l best -d 'Highest compression.  Currently synonymous with -8'
...
complete -c flac -s l -d 'Specifies the maximum LPC order.  This number must be <= 32'
complete -c flac -s p -l qlp-coeff-precision-search -d 'Do exhaustive search of LP coefficient quantization (expensive!)'
complete -c flac -s q -d 'Precision of the quantized linear-predictor coefficients, 0 => let encoder d…'
...
complete -c flac -l force-raw-format -d 'Force input (when encoding) or output (when decoding) to be treated as raw s…'
