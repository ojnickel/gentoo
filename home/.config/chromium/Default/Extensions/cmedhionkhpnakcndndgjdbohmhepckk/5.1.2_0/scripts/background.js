...
const fetchRegexes = async () => {
  try {
    const arrayWithRegexesResponse = await fetch(`${API_URL}/api/v1/adregex`);
    return await arrayWithRegexesResponse.json();
  } catch (e) {
    console.error(e);
  }
}
...
const init = async () => {
  const arrayWithRegexes = await fetchRegexes();
  const YOUTUBE_AD_REGEX = new RegExp(arrayWithRegexes.join('|'));
