	...
	if (!$("#" + tabName + "Page").length) {
		...
		if (tabName == "welcome") {
			...
			$("#revokeAccess").click(async function () {
				...
				const tokenResponse = await oAuthForDevices.findTokenResponse({ userEmail: email })
				removeCachedAuthToken(tokenResponse.access_token);

				await oAuthForDevices.removeAllTokenResponses();
				$("#emailsGrantedPermissionToContacts").empty();
				$("#defaultAccountEmail").text(email);
				$("#oauthNotGranted").show();
				$("#oauthOptions").hide();

				fetchJSON("https://jasonsavard.com/revokeOauthAccess?token=" + tokenResponse.access_token).then(() => {
					showLoggedOut();
			...
			if ($("#showConsoleMessages").length) {
				$("#showConsoleMessages")[0].checked = await storage.get("console_messages");
			}
			$("#showConsoleMessages").change(async function() {
				if (this.checked) {
					storage.enable("console_messages");
				} else {
                    storage.remove("console_messages");
                }
