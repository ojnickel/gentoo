...
async function oauthDeviceSend(sendParams) {
    ...
    return oAuthForDevices.send(sendParams);
}
...
async function updateEvent(params) {
    ...
    if (params.eventEntry && params.eventEntry.sendNotifications && params.eventEntry.sendNotifications != "sent") {
        url = setUrlParam(url, "sendUpdates", "all");
...
async function getArrayOfCalendars() {
	...
	if (calendarList && calendarList.items) {
		calendarList.items.sort(function(calendar1, calendar2) {
...
async function setSnoozeDate(params) {
    ...
    await asyncForEach(params.notifications, async (notification, index) => {
		...
		if (await storage.get("snoozingChangesEventTime") && isCalendarWriteable(getEventCalendar(event)) && isOnline() && (!event.allDay || params.inDays)) {
			...
			if (reminders) {
				...
				let updateEventPromise = updateEvent({
                    event: event,
                    patchFields: patchFields,
                    skipPrompt: true
                }).then(response => {
...
async function openGoogleCalendarWebsite() {
	...
	if (await storage.get("openExistingCalendarTab")) {
		params.urlToFind = "https://calendar.google.com/calendar";
	}
	
	openUrl("https://calendar.google.com/calendar?authuser=" + encodeURIComponent(email), params);
}
...
async function deleteEvent(event, sendNotifications) {
    ...
    let response = await ensureRecurringEventPrompt({event:event});
    if (response.cancel) {
        return response;
    }
...
async function updateCachedFeed(event, params) {
    ...
    if (modifiedCachedFeeds) {
        await storage.set("cachedFeeds", cachedFeeds).then(() => {
...
function getEventUrl(event, email) {
	return event.htmlLink + "&authuser=" + encodeURIComponent(globalThis.email || email);
}
...
async function initOauthAPIs(params = {}) {
    oAuthForDevices = new OAuthForDevices({
        scope: Scopes.CALENDARS_READ + " " + Scopes.EVENTS_READ_WRITE,
