...
async function setEventDateMessage(eventEntry) {
    ...
    const arrayOfCalendars = await getArrayOfCalendars();
	if (eventEntry.calendarId != await getDefaultCalendarId(arrayOfCalendars)) {
...
async function showCreateBubble(params) {
	...
	function initEventEntry() {
        ...
        eventEntry.calendarId = getSelectedCalendarId($("#createEventCalendar"));
	...
	//} else {
		...
		//$createEventDialog.find("#eventDescription").html("");
	//}
                    ...
                    if (foundEvent) {
                        ...
                        openDialog($fbOverlay).then(function (response) {
                            if (response == "ok") {
                                ...
                                eventEntry.calendarId = getSelectedCalendarId($("#fbAddCalendarsMenu"));
...
function getSelectedCalendarId($node) {
	var selectedCalendar = $node[0].selectedItem;
	return $(selectedCalendar).attr("value");
}

function getSelectedCalendar($node) {
	var calendarId = getSelectedCalendarId($node);
...
async function saveQuickAdd() {
	...
	eventEntry.calendarId = getSelectedCalendarId($("#quickAddCalendarsMenu"));
...
async function searchEvents() {
    ...
    const calendarId = getSelectedCalendarId($("#searchCalendarsMenu"));
...
async function displayAgenda(params = {}) {
    ...
    function displayAgendaEvents(eventParams) {
        ...
        $.each(events, (index, event) => {
			...
			if ((eventTitle && (calendarSelected || params.search) && passedShowDeclinedEventsTest(event, showDeclinedEvents, email) && passedHideInvitationsTest(event, hideInvitations, email)) || event.placeHolderForToday) { // && (realEndDay.isToday() || event.endTime.isAfter())
				...
				$eventTitleOnly.append( $("<span class='agendaEventTitle'/>").text(eventTitle) );
				if (eventParams.multipleDayEventsFlag) {
...
async function showDetailsBubble(params) {
	...
	$dialog.find("#clickedEventDelete").off().on("click", async function(e) {
		...
		if (isSnoozer) {
			...
			ensureSendNotificationDialog({ event: event, action: SendNotificationsAction.DELETE}).then(response => {
				if (!response.cancel) {
					showSaving();
					deleteEvent(event, response.sendNotifications).then(async response => {
                        if (!response.cancel) {
                            setStatusMessage({message:getMessage("eventDeleted")});
...
function ensureSendNotificationDialog(params) {
	return new Promise((resolve, reject) => {
		if (params.event.attendees && params.event.attendees.length) {
			if (params.action == SendNotificationsAction.CREATE || (params.event.organizer && params.event.organizer.self)) {
				let content;
...
async function initVisibleCalendarsList() {
    ...
    $.each(arrayOfCalendars, function(index, calendar) {
        ...
        if (isGadgetCalendar(calendar)) {
            ...
            const $displayThisOnly = $("<paper-icon-button class='displayThisCalendarOnly' msgTitle='displayThisOnly' icon='image:remove-red-eye'></paper-icon-button>")
            $displayThisOnly.click(function() {
                ...
                // recheck selected
                let $visibleCalendarCheckbox = $(this).closest(".visible-calendar").find("paper-checkbox");
                $visibleCalendarCheckbox[0].checked = true;

                setSelectedCalendars($visibleCalendarCheckbox.data("calendar"), arrayOfCalendars, true);
            });

            let $visibleCalendar = $("<div class='visible-calendar'></div>");
...
async function init() {
    ...
    let calendarColorsCSS = generateCalendarColors(cachedFeeds, arrayOfCalendars);
    pageVisible.then(() => {
        let cssStyle = addCSS("calendarColors", calendarColorsCSS);
                                        ...
                                        updateEvent(updateEventParams).then(async response => {
                                            if (response.cancel) {
                                                ...
                                                if (eventEntry.recurringEventId) {
                                                    reloadCalendar({source:"recurringEventDragDrop", bypassCache:true, refetchEvents:true});
                                                } else {
