...
function renderMails(params) {
	...
	if (skinsSettings) {
		...
		}) || document.body.classList.contains(`skin_${SkinIds.THEME_DARK}`); // do this check for dynamic night mode
...
function addNightModeIcon($paperItem) {
    const $nightModeIcon = document.createElement("iron-icon");
    $nightModeIcon.id = "nightModeIcon";
    $nightModeIcon.setAttribute("icon", "watch-later"); // image:brightness-2
    $nightModeIcon.setAttribute("style", "margin-left:8px;vertical-align: middle");
...
function showSkinsDialog() {
	...
	Controller.getSkins().then(async skins => {
        ...
        onClickReplace($dialog.querySelector("#night-mode"), async function() {
            if (donationClickedFlag) {
                attemptedToAddSkin = false;

                const $paperItem = selector("#skinsDialog #availableSkins paper-item.iron-selected");
                if ($paperItem) {
                    const skin = getSkin(skins, $paperItem);
                    const nightModeSkin = await storage.get("nightModeSkin");
