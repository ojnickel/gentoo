...
function Account() {
   ...
   async function fetchGmailAT2() {
        // Fetching this will be intercepted by the webrequest get the GMAIL_AT from the cookie and write it to the bg var atByAccount
        const url = that.getMailUrl({useStandardGmailUrl:true, atomFeed:"", urlParams:`timestamp=${Date.now()}&j_get_at`});
        const fetchResponse = await fetchWrapper(url);
        const id = getAccountIdFromUrl(fetchResponse.url);
        const atByAccount = await sendMessageToBG("getAtByAccount");
        return atByAccount[id];
   }
