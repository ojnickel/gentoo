...
function canDisplayImageInNotification(image) {
	const imageRemovedBySender = image.getAttribute("alt")?.includes("Image removed by sender");
	return image.getAttribute("src")?.parseUrl().hostname.includes(".googleusercontent.com") && !imageRemovedBySender;
}
	...
	if (await storage.get("showNotificationEmailImagePreview")) {
		if (await storage.get("accountAddingMethod") == "autoDetect") {
            ...
            const result = await new Promise((resolve, reject) => {
                chrome.permissions.contains({origins: [Origins.NOTIFICATION_IMAGES]}, result => {
                    resolve(result);
                });
            });
            ...
            if (result) {
                if (mail.messages.last()) {
                    ...
                    const image = Array.from(messageContent.querySelectorAll(`img, meta[${DUMMY_ATTRIBUTE}]`)).find(image => {
...
async function showNotification(params = {}) {
    ...
    if (desktopNotification) {
        const dndState = await getDNDState({notificationRelated: true, account: firstEmail.account});
        if (dndState) {
                ...
                if (textNotification || !chrome.notifications) {
                    ...
                    if (notificationFlagForLabelsOfNewestEmail || params.testType) {					
                        ...
                        if (globalThis.Notification) {
                            ...
                            await new Promise((resolve, reject) => {
                                notification = new Notification(title, {body:body, icon: Icons.NOTIFICATION_ICON_URL, requireInteraction: !DetectClient.isMac()});
                    ...
                    if (params.emails.length == 1) {
                        ...
                        if (notificationFlagForLabelsOfNewestEmail || params.testType) {
                            ...
                            let responses;
                            try {
                                responses = await Promise.all([preloadProfilePhoto(email), fetchEmailImagePreview(firstEmail)]);
                            } catch (error) {
                                console.warn("preloadProfilePhotos warn", error);
                            } finally {
                                let fetchEmailImagePreviewResult;
                                if (responses && responses[1]) {
                                    fetchEmailImagePreviewResult = responses[1];
                                }
...
function shortcutNotApplicableAtThisTime(title) {
	...
	notif = new Notification(title, {body:body, icon: Icons.NOTIFICATION_ICON_URL});
    ...
    chrome.webRequest.onBeforeSendHeaders.addListener(
        function(details) {
            ...
            for (var a = 0; a < details.requestHeaders.length; a++) {
                ...
                if ("cookie" === header.name.toLowerCase()) {
                    const thisAt = header.value.split("GMAIL_AT=")[1].split(";")[0];
                    if (details.url.includes("j_get_at")) {
                        const accountId = getAccountIdFromUrl(details.url);
                        atByAccount[accountId] = thisAt;
                    } else {
                        at = thisAt;
                    }
                    break;
        ...
        },
        {
            ...
            urls: [
                ...
                "https://mail.google.com/mail/u/*j_get_at*"
            ]
        },
...
function init() {
	...
	try {
        chrome.storage.local.get(["detectedChromeVersion"], result => {
            if (!result.detectedChromeVersion) {
                ...
                DetectClient.getChromeChannel().then(result => {
                    ...
                    if (result?.channel != "stable") {
                        ...
                        notification = new Notification(title, {body:body, icon: Icons.NOTIFICATION_ICON_URL});
                        notification.onclick = function () {
		...
		if (unsnoozedEmails.length || !lastNotificationAccountDates[accountWithNewestMail.id] || passedDateCheck) {
			...
			if (unsnoozedEmails.length || newestMailObj[accountWithNewestMail.getEmail()] != mostRecentNewEmail.id) {
				...
				if (params.source != Source.STARTUP || (params.source == Source.STARTUP && await storage.get("showNotificationsOnStartup"))) {
					...
					getDNDState({animationRelated: true}).then(dndState => {
						if (!dndState) {
							buttonIcon.startAnimation();
						}
					});
...
function playNotificationSound(source, accountWithNewestMail) {
	return new Promise(async (resolve, reject) => {
        await storage.init();
		...
		getDNDState({soundRelated: true, account: accountWithNewestMail}).then(async dndState => {
			if (dndState || source == "") {
				resolve();
			} else {
